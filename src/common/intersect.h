...
bool PointinTriangle(vec3f A, vec3f B, vec3f C, vec3f P, float& U, float& V)
{
    vec3f v0 = C - A ;
    vec3f v1 = B - A ;
    vec3f v2 = P - A ;
    ...
    for(auto mesh : scene->meshes) {
        ...
        if(mesh->bvh) {
            ...
            for(auto tri : mesh->triangle){
                ...
                // intersect triangle
                vec3f posOnPlane;
                vec3f norm = cross(mesh->pos[tri.y] - mesh->pos[tri.x] , mesh->pos[tri.z] - mesh->pos[tri.x]);
                double vpt = dot(norm,tray.d);
                if(vpt == 0){
                    if(dot(tray.e - mesh->pos[tri.y],norm) != 0)
                        continue;
                    posOnPlane = tray.e;
                    t = 0;
                }else{
                    t = ((mesh->pos[tri.x].x - tray.e.x) * norm.x + (mesh->pos[tri.x].y- tray.e.y) * norm.y + (mesh->pos[tri.x].z - tray.e.z) * norm.z) / vpt;
                    posOnPlane = tray.e + t*tray.d;
                }

                float U,V;
                if(! PointinTriangle(mesh->pos[tri.x], mesh->pos[tri.y], mesh->pos[tri.z], posOnPlane,U,V))
                    continue;
